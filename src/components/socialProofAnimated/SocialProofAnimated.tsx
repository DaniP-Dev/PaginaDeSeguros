...
const SocialProofAnimated = () => {
  return (
    <>
      ...
      <div className="hidden lg:block p-8 bg-gray-50">
        <div className="flex gap-8 animate-marquee whitespace-nowrap">
          {Comentarios.map((comentario) => (
            <div
              key={comentario.id}
              className="bg-white rounded-xl shadow-lg p-8 h-[250px] flex flex-col justify-between text-center"
            >
      ...
      <div className="block lg:hidden p-4 bg-gray-50 overflow-hidden">
        <div className="flex gap-4 animate-marquee whitespace-nowrap w-max">
          {[...Comentarios, ...Comentarios].map((comentario, index) => (
            <div
              key={index}
              className="bg-white rounded-xl shadow-lg px-4 py-3 w-[160px] text-center"
            >
